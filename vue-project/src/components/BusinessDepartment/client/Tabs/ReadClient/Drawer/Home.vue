<template>
  <!-- <a-button type="primary" @click="showDrawer">查看合同</a-button> -->

  <!-- 以下内容，应该 进行动态加载 -->
  <a-drawer v-model:open="open" title="合同列表" width="820" :closable="false" :footer-style="{ textAlign: 'right' }"
    @close="onClose">

    <!-- Tabs -->
    <a-button type="primary" @click="showAddDrawer">添加方案</a-button>
    <PlanAddFormView ref="planaddformref"> </PlanAddFormView>
      <!-- add Drawer -->
      <PlanView></PlanView> 
    <hr>
    <!-- {{ $props.id }} -->
