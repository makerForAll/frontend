...
export interface Party {
  ...
  // ---------------------------------------
  export async function createPartyAPI(data:FrontEndPartyType): Promise<FrontEndPartyType> {
    const response = await axios.post<FrontEndPartyType>(`/party/`,data);
  ...
  export async function readPartyAPI(): Promise<FrontEndPartyType> {
    const response = await axios.get<FrontEndPartyType>('/party');
  ...
  // ---------------------------------------
  export async function updatePartyAPI(id:string,data:FrontEndPartyType): Promise<FrontEndPartyType> {
    // 传给 mysql数据库，需要 把从 前端获取的字符串id 改为 int的id.
    // console.log("传递到 API的 data数据:",id,data);
    const responseObject = await axios.put<FrontEndPartyType>(`/party/${id}`,data);
    // 返回的是一个对象
  ...
  export async function deletePartyAPI(dataID:string): Promise<any> {
    return (await axios.delete<FrontEndPartyType>(`/party/${dataID}`)).data;
