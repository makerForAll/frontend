...
import { type FrontEndPlanType ,readPlanAPI, createPlanAPI, updatePlanAPI ,deletePlanAPI } from '@/api/plan';


export const usePlanStore = defineStore('plan', {
  // 其他配置...
  state: () => ({ 
    readStateData: [] as FrontEndPlanType[],
    currentSelectObjID: '',
  }),  // store 的数据 (data)
  ...
  actions: {  //actions 则是方法 (methods)
    async create(data:any){
      try{
        const createData = await createPlanAPI(data);
    ...
    async read() { // 异步编程获取数据
      try {
        this.readStateData = await readPlanAPI();
        await readPlanAPI();
    ...
    async update(id:string,item: any) {
      try {
        ...
        const updateData = await updatePlanAPI(id,item);
        // console.log("updateData--------------------------------:",updateData);
        // 更新本地 state 中的 party 数据 -------------------------------------------
          //使用 更新数据 中 的ID，来寻找 状态数据中的 party的id对比，找到它在 party 数组里的index值。 
        const index = this.readStateData.findIndex((item:FrontEndPlanType)=> item.id === id);
